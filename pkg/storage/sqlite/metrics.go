...
func (r *metricsRepo) CreateTaskMetrics(ctx context.Context, m TaskMetrics) error {
	query := `INSERT INTO task_metrics (id, task_id, proplet_id, metrics, aggregated, timestamp) VALUES (?, ?, ?, ?, ?, ?)`
...
func (r *metricsRepo) CreatePropletMetrics(ctx context.Context, m PropletMetrics) error {
	query := `INSERT INTO proplet_metrics (id, proplet_id, namespace, cpu_metrics, memory_metrics, timestamp) VALUES (?, ?, ?, ?, ?, ?)`
...
func (r *metricsRepo) ListTaskMetrics(ctx context.Context, taskID string, offset, limit uint64) ([]TaskMetrics, uint64, error) {
	query := `SELECT COUNT(*) FROM task_metrics WHERE task_id = ?`
	...
	query = `SELECT id, task_id, proplet_id, metrics, aggregated, timestamp FROM task_metrics WHERE task_id = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?`
...
func (r *metricsRepo) ListPropletMetrics(ctx context.Context, propletID string, offset, limit uint64) ([]PropletMetrics, uint64, error) {
	query := `SELECT COUNT(*) FROM proplet_metrics WHERE proplet_id = ?`
	...
	query = `SELECT id, proplet_id, namespace, cpu_metrics, memory_metrics, timestamp FROM proplet_metrics WHERE proplet_id = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?`
