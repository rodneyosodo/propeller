...
func (svc *service) invokeBroadcast(ctx context.Context, t task.Task, inputs []string, env map[string]string) (string, error) {
	...
	for i, p := range candidates {
		results, err := svc.invokeOnProplet(ctx, t, p.ID, inputs, env)
		if err == nil {
			return results, nil
		}
...
func (svc *service) recoverLatentDeployment(ctx context.Context, t task.Task, proplets []proplet.Proplet) {
	for _, p := range proplets {
		if err := svc.redeployLatent(ctx, t, p.ID); err != nil {
			svc.logger.WarnContext(ctx, "failed to re-deploy latent task", "task_id", t.ID, "proplet_id", p.ID, "error", err)
		}
	}
}
