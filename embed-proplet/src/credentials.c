...
static int creds_set_cb(const char *key, size_t len, settings_read_cb read_cb,
  ...
  if (strcmp(key, "wifi_ssid") == 0) {
    ...
    size_t max = CRED_MAX_LEN - 1;
    ssize_t rc = read_cb(cb_arg, g_creds->api_key, MIN(len, max));
    if (rc < 0)
      return (int)rc;
    g_creds->api_key[rc] = '\0';
  } else if (strcmp(key, "tenant_id") == 0) {
    size_t max = CRED_MAX_LEN - 1;
    ssize_t rc = read_cb(cb_arg, g_creds->tenant_id, MIN(len, max));
    if (rc < 0)
      return (int)rc;
    g_creds->tenant_id[rc] = '\0';
...
const struct proplet_credentials *credentials_get(void) { return g_creds; }
