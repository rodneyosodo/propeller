...
int main(void) {
  ...
  if (mqtt_client_connect(creds.tenant_id, creds.proplet_id, creds.api_key,
                          creds.channel_id) != 0) {
    LOG_ERR("MQTT connect failed");
    return -1;
  }

  if (subscribe(creds.tenant_id, creds.channel_id) != 0) {
    LOG_ERR("MQTT subscribe failed");
    return -1;
  }

  if (publish_discovery(creds.tenant_id, creds.proplet_id, creds.channel_id,
                        PROPLET_DESCRIPTION, PROPLET_TAGS, PROPLET_LOCATION,
                        PROPLET_VERSION) != 0) {
  ...
  while (1) {
    ...
    if (now >= next_alive) {
      publish_alive_message(creds.tenant_id, creds.channel_id);
      next_alive = now + PROPLET_LIVELINESS_INTERVAL_MS;
    }

    if (now >= next_metrics) {
      publish_metrics_message(creds.tenant_id, creds.channel_id,
                              creds.proplet_id, PROPLET_NAMESPACE);
      next_metrics = now + PROPLET_METRICS_INTERVAL_MS;
    }

    if (now >= next_task_metrics) {
      publish_active_task_metrics(creds.tenant_id, creds.channel_id,
                                  creds.proplet_id);
