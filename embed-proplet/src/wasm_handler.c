...
static bool write_string_to_wasm_memory(wasm_exec_env_t exec_env,
                                        const char *str, uint32_t *ret_offset,
                                        uint32_t *ret_len) {
  ...
  uint32_t str_buf_offset =
      wasm_runtime_module_malloc(module_inst, str_len + 1, (void **)NULL);
  if (str_buf_offset == 0) {
...
static bool get_proplet_id_wrapper(wasm_exec_env_t exec_env,
  ...
  const char *proplet_id =
      g_current_task.proplet_id[0] != '\0' ? g_current_task.proplet_id : "";
...
static bool get_model_data_wrapper(wasm_exec_env_t exec_env,
  ...
  const char *model_data =
      g_current_task.model_data_fetched && g_current_task.model_data[0] != '\0'
...
static bool get_dataset_data_wrapper(wasm_exec_env_t exec_env,
                                     uint32_t *ret_offset, uint32_t *ret_len) {
  const char *dataset_data = g_current_task.dataset_data_fetched &&
                                     g_current_task.dataset_data[0] != '\0'
                                 ? g_current_task.dataset_data
                                 : "";
  return write_string_to_wasm_memory(exec_env, dataset_data, ret_offset,
                                     ret_len);
}
...
void execute_wasm_module(const char *task_id, const uint8_t *wasm_data,
  ...
  if (!module) {
    ...
    publish_results_with_error(tenant_id, channel_id, task_id, NULL, error_buf);
  ...
  if (!wasm_runtime_register_natives("env", native_symbols, n_native_symbols)) {
    LOG_WRN("Failed to register native symbols, host functions may not be "
            "available");
  }
  ...
  if (!wasm_runtime_call_wasm_a(exec_env, func, result_count, results, n_args,
    ...
    extern const char *channel_id;
    extern const char *tenant_id;
    publish_results_with_error(tenant_id, channel_id, task_id, NULL, error_msg);
  } else {
    ...
    extern const char *channel_id;
    extern const char *tenant_id;
    publish_results(tenant_id, channel_id, task_id, results_string);
