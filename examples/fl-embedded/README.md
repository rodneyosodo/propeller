...
- Go 1.21+
- Propeller infrastructure running (see main `docker/compose.yaml`)
...
1. **Manager** creates task and publishes to Propeller MQTT topic: `m/{tenant_id}/c/{channel}/control/manager/start`
...
4. **Update Submission**:
   ...
   - Proplet publishes to Propeller MQTT: `fl/rounds/{round_id}/updates/{proplet_id}`
...
Subscribe to the FL updates topic (connects to Propeller MQTT adapter):
...
2. **Update Submission**: Embedded proplet uses MQTT directly (via Propeller)
