...
This guide covers running the production-grade Federated Learning (FL) demo application using the full Propeller stack.
...
- `docker/.env` file with Propeller configuration
...
4. **[Provision Propeller Resources [ONE-TIME]](#step-4-provision-propeller-resources-one-time)** - Create tenant, channel, and clients (one-time setup)
...
**IMPORTANT**: When configuring FL experiments, you must use **Propeller ENTITY_IDs** (UUIDs), not instance IDs.

- **Instance IDs**: `"proplet-1"`, `"proplet-2"`, `"proplet-3"` - These are just labels for identification
- **ENTITY_IDs**: `"3fe95a65-74f1-4ede-bf20-ef565f04cecb"` - These are the actual Propeller client credentials that proplets use to register with the manager
...
- `propeller-coordinator-http` - Coordinator with MQTT authentication support
...
- Full Propeller production stack (Auth, Domains, Clients, Channels, RabbitMQ, NATS, MQTT Adapter, Nginx)
- FL-specific services (Model Registry, Aggregator, Local Data Store, Coordinator)
- Propeller services (Manager, Proplets, Proxy)

## Step 4: Provision Propeller Resources [ONE-TIME]

**One-time setup**: This creates persistent data in Propeller databases. Only needs to be repeated if you remove Docker volumes (e.g., `docker compose down -v`).

**IMPORTANT**: Before the manager and proplets can connect to MQTT, you must provision the necessary Propeller resources (tenant, channel, and clients).
...
- `MANAGER_ENTITY_ID` and `MANAGER_API_KEY`
- `PROPLET_ENTITY_ID` and `PROPLET_API_KEY` (for proplet-1)
- `PROPLET_2_ENTITY_ID` and `PROPLET_2_API_KEY` (for proplet-2)
- `PROPLET_3_ENTITY_ID` and `PROPLET_3_API_KEY` (for proplet-3)
- `COORDINATOR_ENTITY_ID` and `COORDINATOR_API_KEY`
- `PROXY_ENTITY_ID` and `PROXY_API_KEY`
...
```bash
# IMPORTANT: Export ENTITY_IDs from docker/.env (Propeller client IDs, NOT instance IDs)
...
> - Get the current client ID and key from `docker/.env` (MANAGER_ENTITY_ID and MANAGER_API_KEY) or from the provisioning script output.
...
- **Step 4**: Provision Propeller Resources - Only needed once unless you run `docker compose down -v` (removes volumes)
...
4. **Verify coordinator MQTT credentials**: Ensure `COORDINATOR_ENTITY_ID` and `COORDINATOR_API_KEY` are set in `docker/.env` (updated by provisioning script)
...
**Root Cause**: The `participants` array in ConfigureExperiment is using instance IDs (`"proplet-1"`, `"proplet-2"`, `"proplet-3"`) instead of Propeller ENTITY_IDs (UUIDs).
...
**Why this happens**: Proplets register themselves with their Propeller ENTITY_ID (UUID), not their instance ID. The manager looks up proplets by the ID they registered with, so you must use ENTITY_IDs in the participants array.
...
4. **Verify proxy has correct tenant/channel IDs**:
   ...
   ```bash
   grep -E '^(PROXY_TENANT_ID|PROXY_CHANNEL_ID|PROXY_ENTITY_ID|PROXY_API_KEY)=' docker/.env
   ```
...
1. **Add proxy configuration to `docker/.env`**:
   ...
   PROXY_API_KEY=<YOUR_PROXY_API_KEY>  # Replace with your actual proxy client key (UUID format)
   ...
   > **Note**: Replace all placeholders (`<YOUR_TENANT_ID>`, `<YOUR_CHANNEL_ID>`, `<YOUR_PROXY_ENTITY_ID>`, `<YOUR_PROXY_API_KEY>`, `YOUR_GITHUB_USERNAME`, `ghp_xxxxx`) with your actual values. These values are typically generated by the provisioning script (Step 4) or can be obtained from your `docker/.env` file after provisioning.
