(module
  ...
  (func $main (param i32 i32) (result i32)
    ...
    (block $outer_break
      (loop $outer
        (br_if $outer_break (i32.ge_u (local.get $round) (i32.const 100)))
        (local.set $i (i32.const 0))
        (block $write_break
          (loop $write
            (br_if $write_break (i32.ge_u (local.get $i) (i32.const 4096)))
        ...
        (block $read_break
          (loop $read
            (br_if $read_break (i32.ge_u (local.get $i) (i32.const 4096)))
            (local.set $acc
